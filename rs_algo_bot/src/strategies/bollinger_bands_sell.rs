...
#[derive(Clone)]
pub struct BollingerBandsSell<'a> {
    name: &'a str,
...
impl<'a> Strategy for BollingerBandsSell<'a> {
    fn new(
        ...
        let name = name.unwrap_or("Bollinger_Bands_Sell");
    ...
    fn trading_direction(
        ...
        self.trading_direction = time_frame::get_htf_trading_direction(
            ...
            |(idx, _prev_idx, htf_inst)| {
                ...
                let is_long = htf_ema_a > htf_ema_b; // && *rsi >= 20. && *rsi <= 50.;
                let is_short = htf_ema_a < htf_ema_b; // && *rsi >= 50. && *rsi <= 80.;
