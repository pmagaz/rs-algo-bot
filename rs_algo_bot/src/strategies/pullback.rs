...
impl<'a> Strategy for Pullback<'a> {
    fn new(
    ...
    fn entry_long(
        ...
        let anchor_htf = time_frame::get_htf_data(
            ...
            |(idx, _prev_idx, htf_inst)| {
                ...
                let htf_ema_c = htf_inst.indicators.ema_c.get_data_a().get(idx).unwrap();
                htf_ema_a > htf_ema_b && htf_ema_b > htf_ema_c && close_price > htf_ema_b
            },
        );
        ...
        let entry_condition = anchor_htf && (close_price < ema_c && close_price > ema_c);
