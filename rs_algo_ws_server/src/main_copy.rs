...
fn handle_message(msg: Message, broker: &mut Xtb) -> Option<String> {
    let data = match msg {
        Message::Ping(bytes) => {
...
async fn handle_session(sessions: Sessions, raw_stream: &mut TcpStream, addr: SocketAddr) {
    ...
    loop {
        ...
        let broadcast_incoming = incoming.try_for_each(|msg| {
            ...
            match handle_message(msg, &mut broker) {
                Some(msg) => send_message(sessions.clone(), &addr, Message::Text(msg)),
